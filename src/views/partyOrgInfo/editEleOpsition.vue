<template>
  <div class="add-chapter">
    <div class="add-chapter-title">编辑职位</div>
    ...
    <button @click="updateAddChapter" class="add-chapter-add">保存修改</button>
  </div>
</template>

<script>
import { getOrgPositionId, postUpdateOrgPosition } from "./../../api/api";
export default {
  name: "editEleOpsition",
  data() {
    return {
      chapterName: ""
    };
  ...
  mounted() {
    getOrgPositionId({
      ...
      if (res.code == 200) {
        this.chapterName = res.data.postion_name;
  ...
  methods: {
    closeChapter: function() {
      this.$parent.closeEditeChapter();
    },
    updateAddChapter: function() {
      if (this.chapterName === "") {
        alert("请输入职位名称！");
      } else {
        postUpdateOrgPosition({
          id: this.ID,
