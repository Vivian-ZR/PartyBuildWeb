  ...
  <div>
    ...
    <div class="user-message-add-body">
      <div class="user-message-add-from">
        ...
        <div class="ele-archives-title">编辑组织</div>
        ...
        <button @click="clickReturn" class="user-message-info-button add-party-user-button1">取消修改</button>
        <button @click="saveEle" class="user-message-info-button add-party-user-button2">保存修改</button>
      </div>
    </div>
  </div>
</template>
...
import { getUserlist, postUpdateOrganizationBasics, getOrganizationBasics } from "../../api/api";
export default {
  name: "editEle",
  data() {
    return {
        ...
        getOrganizationBasics({
          ...
          if (res.code == 200) {
            (this.organizationName = res.data.organizationName),
              (this.adminName = res.data.organizationAdmin),
              (this.workAddres = res.data.officeLocation),
              (this.organizationSynopsis = res.data.organizationSynopsis);
            for (var i = 0; i < this.adminList.length; i++) {
              if (this.adminName === this.adminList[i].user_name) {
                this.positionId = this.adminList[i].id;
              }
            }
